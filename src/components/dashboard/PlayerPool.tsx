...
function PoolPlayerCard({ player }: { player: Player }) {
  ...
  return (
    ...
    >
      <span className="font-medium text-neutral-900 dark:text-neutral-100">{player.name}</span>
...
export function PlayerPool({ players }: { players: Player[] }) {
  ...
  return (
    ...
    >
      ...
      {players.length === 0 && (
        <p className="text-sm text-neutral-400 dark:text-neutral-600">Every player has a position.</p>
      )}
      {players.map((player) => (
